...
\begin{figure}
\includegraphics[width=0.9\columnwidth]{atom_fig4.eps}
\includegraphics[width=0.9\columnwidth]{atom_fig5.eps}
...
\caption{Fidelity evolution for different atomic separation. The atomic separations of $\lambda_0$, $0.1\lambda_0$, $0.2\lambda_0$ are plotted. Squeezing parameter $r=1$ and time unit $\tau=1/\sqrt{\gamma_1\gamma_2}$ is the geometric mean of the transition $|a\rangle\rightarrow|b\rangle$ and $|b\rangle\rightarrow|c\rangle$'s spontaneous emission rate in ordinary vacuum.}
\label{3}
\end{figure}
...
We study the $\Xi$-type atoms coupled to a broadband squeezed vacuum reservoir in a quasi-one-dimensional waveguide, with the overall transition frequency $\omega_{ac}=2\omega_0$. We show that the atoms can evolve into a steady state where each atom is in the superposition state of the ground state and the second excited state even if dipole-dipole interaction between atoms exist. If the decay rate from the second excited state to the first excited state is much smaller than that from the first excited state to the ground state, the population can be almost trapped in the second excited state. This may be useful for two-photon laser.  
