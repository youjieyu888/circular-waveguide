...
\begin{abstract}
We study the steady state of $\Xi$-type atoms coupled to the squeezed vacuum reservoir in the quasi-one-dimensional waveguide. We show that the system evolves into a pure state where the dephasing rate is zero. What's more, the population is inverted in the steady state when the coupling strength of two transitions of the $\Xi$-type atom to the squeezed vacuum are properly set. We prove in principle that these results can be generalized to multiple interactive atoms.
\end{abstract}
...
In our study, we consider the $\Xi$-type atoms coupled to the broadband squeezed vacuum in the quasi-one-dimensional waveguide. We found that the steady state is a pure state with zero dephasing rate, and the population distribution can be inverted. We also demonstrate that the complete population inversion can be achieved with proper parameters.
...
>>>>>>> ef395fa2071054631a8224102b987d90b543219a
\begin{widetext}
\begin{equation}
\label{eq1}
\begin{split}
...
\end{split}
\end{equation}
where $\gamma_{i}$ is the decay rate for transition $i$ in ordinary vacuum. Here we have incorporated index $l$ into $i$ for compactness, so $i$ denotes transition $ab$ for the first atom, transition $bc$ for the first atom, transition $ab$ for the second atom, transition $bc$ for the second atom, etc. For simplicity, we assume all atoms are along $z$-axis such that $\vec{r_i}=(0,0,r_i)$,
...
\end{split}
\end{equation}
Since we need to sum over $l,m,i,j$, the imaginary part of $e^{sgn(2r_{c}-o_{1}-o_{2})ik_{0}(r_{l}-r_{m})}$ gets canceled, which yields $\gamma'_{ij}=\sqrt{\gamma_{i}\gamma_{j}}\cos[k_{0z}(r_{ij})]$. The above calculation is also valid when $r_i=r_j$.
